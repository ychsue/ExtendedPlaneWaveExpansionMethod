function interface2omega(varargin)
...
figure;plot([omegam;omegam]', [TTEm;TTMm]');title('TE and TM');
legend('TE','TM');
figure;plot([omegam;omegam]', [checkEm;checkMm]');title('check sum');
time=now-time
time=time*86400
inputdata=varargin;
if length(folder)~=0
  eval(['save ',folder,'/interface2omega',printdata(varargin{:}) ,' checkEm checkMm RTEm TTEm RTMm TTMm K ky kTE kTM VTE VTM ep1 inputdata']);
end