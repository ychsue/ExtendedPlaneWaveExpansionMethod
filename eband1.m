...
%Get the \epslon(K)
epm1=epslonm1(Kx-Kxp,Ky-Kyp,fftpattern);
...
for l=0:0.25:1.5;
    ...
    [HkTM,omegaTM2]=eig((kk+K)*((kk+Kp)').*epm1);
    [HkTE,omegaTE2]=eig(diag(diag((kk+K)*((kk+Kp)'))),inv(epm1));
...
time=now-time
inputdata=varargin;
eval(['save data\eband1',printdata(varargin{:}) ,' omegaTE omegaTM epm1 inputdata']);